    ...
    <div className="p-7">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-1">{totalItems} items tracked</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="uniweb-card p-4 relative overflow-hidden">
          ...
          <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Total Items</div>
          <div className="text-2xl font-bold text-foreground mt-1">{totalItems}</div>
        </div>
        <div className="uniweb-card p-4 relative overflow-hidden">
          <div className="kpi-stripe bg-status-red" />
          <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Low Stock</div>
          <div className="text-2xl font-bold text-destructive mt-1">{lowStockCount}</div>
        </div>
        <div className="uniweb-card p-4 relative overflow-hidden">
          <div className="kpi-stripe bg-status-green" />
          <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Stock Value</div>
          <div className="text-2xl font-bold text-foreground mt-1 font-mono">${totalValue.toFixed(0)}</div>
        </div>
        <div className="uniweb-card p-4 relative overflow-hidden">
          <div className="kpi-stripe bg-status-amber" />
          <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Active POs</div>
          <div className="text-2xl font-bold text-foreground mt-1">{purchaseOrders.filter(p => p.status === "submitted").length}</div>
        </div>
      </div>
      ...
      <Tabs defaultValue="stock" className="space-y-4">
        ...
        <TabsContent value="stock">
          <div className="relative w-72 mb-4">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search inventory..." className="w-full h-10 pl-10 pr-4 rounded-[9px] bg-card border-[1.5px] border-border text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-[3px] focus:ring-primary/10 transition-all" />
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="table-row border-b border-border/50 last:border-b-0 hover:bg-accent/30">
                      <td>
                        <div className="font-semibold text-foreground">{item.name}</div>
                        {item.nameZh && <div className="text-[11px] text-muted-foreground">{item.nameZh}</div>}
                      </td>
                      <td><span className="status-badge bg-accent text-muted-foreground">{item.category}</span></td>
                      <td className="w-40">
                      ...
                      <td className="font-mono">${item.costPerUnit.toFixed(2)}</td>
                      <td className="text-muted-foreground text-[12px]">{item.supplier || "—"}</td>
                      <td>
                        {adjustingId === item.id ? (
                          <div className="flex items-center gap-1">
                            <input value={adjustQty} onChange={e => setAdjustQty(e.target.value)} placeholder="+/-" className="w-16 h-8 px-2 rounded-md border border-border text-[12px] text-center bg-background" />
        ...
        <TabsContent value="log">
          <div className="uniweb-card overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {movements.map(mv => (
                  <tr key={mv.id} className="table-row border-b border-border/50 last:border-b-0">
                    <td className="mono text-muted-foreground">{new Date(mv.timestamp).toLocaleString("en-SG", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</td>
                    <td className="font-semibold text-foreground">{mv.itemName}</td>
                    <td><span className={cn("status-badge", mv.type === "restock" ? "bg-status-green-light text-status-green" : mv.type === "waste" ? "bg-status-red-light text-status-red" : "bg-accent text-muted-foreground")}>{mv.type}</span></td>
                    <td className={cn("font-mono font-semibold", mv.quantity > 0 ? "text-status-green" : "text-destructive")}>{mv.quantity > 0 ? "+" : ""}{mv.quantity}</td>
                    <td className="text-muted-foreground text-[12px]">{mv.note || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
